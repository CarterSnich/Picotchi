...
void loop() {
  delay(40);
  KeypadAstate = digitalRead(KEYPAD_A);
  ...
  if (!dead) {
    ...
    /* ------- KEYPAD B - BACK ------- */
    if (KeypadBState == LOW && menuOpened) {
      delay(40);
      if (soundEnabled) {
    ...
    /* KEYPAD UP  */
    if (KeypadUpState == LOW && menuOpened) {
      if (menuDepth) {
        if (subMenu > 1) {
          --subMenu;
        }
    ...
    /* KEYPAD DOWN  */
    if (KeypadDownState == LOW && menuOpened) {
      if (menuDepth) {
      ...
      if (!gameOver && !paused) {
        ...
        if (!jumping && ((obstacle1show && display.width() - obstacle1XPos >= 20 && display.width() - obstacle1XPos <= 46) || (obstacle2show && display.width() - obstacle2XPos >= 20 && display.width() - obstacle2XPos <= 46))) {
          ...
          if (soundEnabled) {
            ...
            delay(50);
            tone(SPEAKER_PIN, 350, 40);
            delay(50);
            tone(SPEAKER_PIN, 200, 60);
            noTone(SPEAKER_PIN);
          }
